...
//    	setAttrToDoList(username);//获取待办
    	setAttrHavedoneList(username);//获取已办
    	setAttr("NoticeList",noticeService.getMyNotice(user.getId()));//通知公告
		render("haveTodoList.html");
	}


	/****
	 * 获取申请办理任务页面
	 * defkey~！！！！！！！！！！！待整理  oa_leave_v1
	...
	public void getDoTaskPage() throws ClassNotFoundException, NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException{
		...
		if(StrKit.notBlank(taskid)){
			...
			try {
				//判断是否要重新编辑
				if("ReEdit".equals(task.getTASKDEFKEY())){
					String className = WorkFlowUtil.getClassFullNameByDefKey(task.getDEFKEY());
					Class<?> userClass = Class.forName(className);
					//模型名称
					setAttr("formModelName",StringUtil.toLowerCaseFirstOne(userClass.getSimpleName()));
					setAttr("view", "reEdit");
				}else{
					setAttr("view", "detail");
				}
			...
			Record o = new Record();
			//如果属于固定流程
			if(StrKit.notBlank(WorkFlowUtil.getTablenameByDefkey(defKey))){
	...
	public void setPageUrl(String defKey,Record o){
		...
		if(StrKit.notBlank(WorkFlowUtil.getTablenameByDefkey(defKey))){
			//财务类的
    		if(defKey.indexOf(OAConstants.DEFKEY_APPLY_FINANCE)==0){
    			setAttr("pageUrl", "/WEB-INF/admin/oa/apply/finance/editForm.html");
    		}
    		// 请假类申请
			if(defKey.indexOf(OAConstants.DEFKEY_APPLY_LEAVE)==0){
    			setAttr("pageUrl","/WEB-INF/admin/oa/apply/leave/editForm.html");
			}
		}
	}
